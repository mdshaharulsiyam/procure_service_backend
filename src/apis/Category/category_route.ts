...
category_router
    .post('/category/create', verifyToken(config.ADMIN), asyncWrapper(category_controller.create))

    .get('/category/get-all', asyncWrapper(category_controller.get_all))

    .patch('/category/update/:id', verifyToken(config.ADMIN), asyncWrapper(category_controller.update))
