...
async function sing_in(req: Request, res: Response) {
    ...
    sendResponse(
        ...
        HttpStatus.SUCCESS,
        result,
        [config.TOKEN_NAME, result?.token, 60 * 60 * 24 * 500 * 1000]
    )
}

async function sing_out(req: Request, res: Response) {
    res.clearCookie(config.TOKEN_NAME);
    res.cookie(config.TOKEN_NAME, "", {
        maxAge: 0,
        expires: new Date(),
        sameSite: 'none',
        httpOnly: false,
        secure: false
    }).status(200).send({ success: true, message: "user sign out successfully" })
}
...
async function reset_password(req: Request, res: Response) {
    ...
    const result = await auth_service.reset_password(req?.body, req?.user as IAuth)
...
async function change_password(req: Request, res: Response) {
    ...
    const result = await auth_service.change_password(req?.body, req?.user as IAuth)
