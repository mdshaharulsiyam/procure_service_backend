...
const auth_schema = new Schema<IAuth>(
    {
        ...
        documents: {
            ...
            validate: [
                {
                    validator: function (value: string[]) {
                        return this.role !== 'PROFESSIONAL' || (Array.isArray(value) && value.length === 2);
                    },
                    message: 'Verification must have exactly two elements: [front, back] for PROFESSIONAL role only',
                },
                {
                    validator: function (value: string[]) {
                        return this.role !== 'PROFESSIONAL' || value.every((v) => typeof v === 'string' && v.trim() !== '');
                    },
                    message: 'Each verification element must be a non-empty string for PROFESSIONAL role only',
                },
            ],
        },
