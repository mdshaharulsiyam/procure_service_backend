...
async function get_all(queryKeys: QueryKeys, searchKeys: SearchKeys, populatePath?: any, selectFields?: string | string[], modelSelect?: string) {
    return await Queries(notification_model, queryKeys, searchKeys, populatePath, selectFields, modelSelect)
}
async function read_notification(id: string, user: IAuth) {
    const result = await notification_model.findOneAndUpdate({ _id: id, user }, {
        $set: {
...
export const notification_service = Object.freeze({
    create, get_all, read_notification
})