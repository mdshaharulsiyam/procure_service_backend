...
async function get_all(req: Request, res: Response) {
    ...
    const result = await notification_service.get_all(queryKeys, searchKeys, populatePath, selectFields, modelSelect)
    sendResponse(
        res,
        HttpStatus.SUCCESS,
        result
...
async function read(req: Request, res: Response,) {
    ...
    const result = await notification_service.read_notification(req?.params?.id, req?.user as IAuth)
