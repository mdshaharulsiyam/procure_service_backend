...
service_router
    .post('/service/create', verifyToken(config.ADMIN), asyncWrapper(category_controller.create))

    .get('/service/get-all', asyncWrapper(category_controller.get_all))

    .patch('/service/update/:id', verifyToken(config.ADMIN), asyncWrapper(category_controller.update))
